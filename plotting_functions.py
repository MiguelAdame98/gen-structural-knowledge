...
def square_plot(cells, env, pars, plot_specs, name='sq', lims=(), mask=False, env_class=None, fig_dir=None):
    ...
    # Debug: Check cell and shape
    print("Cell shape:", np.shape(cell))

    xs, ys = env_class.get_node_positions(_plot_specs=plot_specs, _mask=mask)
    print("Node positions (xs, ys):", xs, ys)  # Debug
    ...
    for grid in range(n):
        ...
        print("Grid:", grid, "xs:", xs, "ys:", ys, "cell_prepared:", cell_prepared)  # Debug

        plt.subplot(n_rows, n_cols, grid + 1)
        plt.scatter(xs, ys, c=cell_prepared, cmap=plot_specs.cmap, s=75, marker='o')
    plt.tight_layout()
